...
function renderFeed(filterTag = null) {
  ...
  visiblePosts.forEach(post => {
    ...
    card.innerHTML = `
      ...
      <p style="font-size: small;color: grey;">${new Date(post.timestamp).toLocaleString()}</p>
      <p>${post.content || ''}</p>
      ${imageHTML}
      <div class="post-footer">
        ${currentUser ? `<button class="like-btn ${liked ? 'liked' : ''}" data-id="${post.id}">${post.likes} 赞</button>` : ''}
        <a href="detail.html?id=${post.id}" style="padding-left: 10px;text-decoration: none;color: grey;font-size: 0.9rem;">查看详情</a>
        ${currentUser ? `<a href="javascript:void(0);" class="toggleComment" data-id="${post.id}" style="padding-left: 10px;text-decoration: none;color: grey;font-size: 0.9rem;"> 评论</a>
             <a href="#" onclick="sharePost('${post.id}')" style="padding-left: 10px;text-decoration: none;color: grey;font-size: 0.9rem;">转发</a>
        <a href="#" onclick="alert('已收藏！')" style="padding-left: 10px;text-decoration: none;color: grey;font-size: 0.9rem;">收藏</a>
        `: ''}
        ${isAuthor ? `
        <a href="edit.html?id=${post.id}" style="padding-left: 10px;text-decoration: none;color: grey;font-size: 0.9rem;">编辑</a>
        <a href="#" class="deletePost" data-id="${post.id}" style="padding-left: 10px;text-decoration: none;color: grey;font-size: 0.9rem;">删除</a>
        ` : ''}

        ${currentUser ? `
        <div id="commentBox-${post.id}" style="margin-top:10px; display:none;">
